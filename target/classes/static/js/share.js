...
share.switcher = {
	...
	reshowChecked : function() {
		var swts = $(".js-switch");
		var that = this;
		this.checked.forEach(function(value) {
			that.changeMod(value, true);
		});
		this.checked = [];
	},
	changeMod : function(id, check) {
		...
		if (mod.substring(0, 3) == 'map') {
			...
			var mod = mapLayer.getMarkerMod(name);
			if (check) {
				mapLayer.showMarkers(mod);
			} else {
				mapLayer.hideMarkers(mod);
			}
		} else {
			if (check) {
...
share.console = {
	...
	change : function() {
		...
		if (link == "") {
			// 回复console界面地图的设置
			share.switcher.reshowChecked();

			this.changeSet("set-console");
			console.log("console");
		} else {// 其他页面
			// 保存地图上的地图模块
			share.switcher.saveChecked();
			link = link.substring(1, link.length);
			if (link == "anaylyze_heat") {
				this.changeSet("set-visual");
				console.log("analyze heat");
			}
		}
